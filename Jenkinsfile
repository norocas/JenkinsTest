    ...
    stages {
        stage('Build') {
            ...
            steps {
                ...
                stash(name: 'compiled-results', includes: 'pytest/*.py*')
            }
        }
        stage('Test') {
            agent {
            ...
            steps {
                ...
                sh 'py.test --verbose --junit-xml test-reports/results.xml pytest/test_add.py'
            }
            post {
                always {
        ...
        stage('Deliver') {
                    ...
                    steps {
                        ...
                        dir(path: env.BUILD_ID) {
                            ...
                            sh "docker run --rm -v ${VOLUME} ${IMAGE} 'pyinstaller -F mod_add.py'"
                        }
                    }
                    post {
                        success {
                            ...
                            archiveArtifacts "${env.BUILD_ID}/sources/dist/mod_add"
